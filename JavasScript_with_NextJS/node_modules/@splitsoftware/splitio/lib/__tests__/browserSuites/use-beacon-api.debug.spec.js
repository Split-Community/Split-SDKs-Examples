...
var sendBeaconSpyDebug; // util method to trigger 'unload' event
...
var assertCallsToBeaconAPI = function assertCallsToBeaconAPI(assert) {
  assert.ok(sendBeaconSpyDebug.calledTwice, 'sendBeacon should have been called twice'); // The first call is for flushing impressions

  var impressionsCallArgs = sendBeaconSpyDebug.firstCall.args;
  ...
  assert.equal(parsedPayload.sim, _constants.DEBUG, 'assert correct impressions mode');
  assertImpressionSent(assert, parsedPayload.entries[0]); // The second call is for flushing events

  var eventsCallArgs = sendBeaconSpyDebug.secondCall.args;
...
function beaconApiNotSendTestDebug(fetchMock, assert) {
  sendBeaconSpyDebug = _sinon.default.spy(window.navigator, 'sendBeacon'); // Mocking this specific route to make sure we only get the items we want to test from the handlers.
...
var _default = beaconApiNotSendTestDebug;
