...
function _default(config, fetchMock, assert) {
  var i = 0,
      tested = 0;
  var wBucketing = !!config.core.key.bucketingKey;

  var getTreatmentTests = function getTreatmentTests(client, TAClient) {
    ...
    assert.equal(client.getTreatment('qc_team'), 'no'); // If we are with the bucketing key, we should get a different treatment.
    ...
    }), 'off'); // This is an special case for the browser.
    ...
    assert.equal(client.getTreatment('hierarchical_splits_test'), 'on'); // This split has a traffic allocation of 1 (lowest) and the key we're using also returns the lowest bucket for TA (1)
    ...
    assert.equal(TAClient.getTreatment('ta_bucket1_test'), 'rollout_treatment'); // With a higher bucket it's ok to get default treatment

    assert.equal(client.getTreatment('ta_bucket1_test'), 'default_treatment');
  };

  var getTreatmentsTests = function getTreatmentsTests(client) {
    assert.deepEqual(client.getTreatments([// Treatments List
    'blacklist', 'whitelist', 'splitters', 'qc_team', 'hierarchical_splits_test']), {
      // Expected result
    ...
    assert.deepEqual(client.getTreatments(['employees_between_21_and_50_and_chrome', 'user_attr_gte_10_and_user_attr2_is_not_foo', 'user_account_in_whitelist']), {
  ...
  var getTreatmentsWithConfigTests = function getTreatmentsWithConfigTests(client) {
    ...
    assert.deepEqual(client.getTreatmentWithConfig('not_existent_split'), {
      treatment: 'control',
      config: null
    ...
    assert.deepEqual(client.getTreatmentsWithConfig(['errored', null, 'errored', 'something'], function () {}), {
    ...
    assert.deepEqual(client.getTreatmentsWithConfig(['not_existent', 'other']), {
      not_existent: CONTROL_WITH_CONFIG,
      other: CONTROL_WITH_CONFIG
    }, 'If I get a treatment for non existent Splits, I get control as treatment and config null for those split names.');
  ...
  var _loop = function _loop() {
    ...
    fetchMock.getOnce('https://sdk.split.io/api/mySegments/aaaaaaklmnbv', {
      status: 200,
      body: {
        mySegments: []
      }
    ...
    var clientTABucket1 = splitio.client('aaaaaaklmnbv');
