...
function ipAddressesSettingAssertions(fetchMock, assert) {
  ...
  function assertImpression(IPAddressesEnabled, Mode, impression) {
    assert.equal(impression.ip, IPAddressesEnabled ? IP_VALUE : Mode === _constants.STANDALONE_MODE ? false : NA, "If IPAddressesEnabled, \"ip\" property in impressions must be equal to the machine ip. If not, it must be equal to \"".concat(NA, "\" for \"").concat(_constants.CONSUMER_MODE, "\" mode\" or false for \"").concat(_constants.STANDALONE_MODE, "\" mode."));
    assert.equal(impression.hostname, IPAddressesEnabled ? HOSTNAME_VALUE : Mode === _constants.STANDALONE_MODE ? false : NA, "If IPAddressesEnabled, \"hostname\" property in impressions must be equal to the machine hostname. If not, it must be equal to \"".concat(NA, "\" for \"").concat(_constants.CONSUMER_MODE, "\" mode\" or false for \"").concat(_constants.STANDALONE_MODE, "\" mode."));
