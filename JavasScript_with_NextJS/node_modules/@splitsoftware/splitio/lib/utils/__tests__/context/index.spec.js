...
var _constants = _interopRequireDefault(require("../../context/constants"));
...
**/
(0, _tapeCatch.default)('CONTEXT module', function (t) {
  t.test('Should have an API for storing and retrieving items', function (assert) {
    assert.equal((0, _typeof2.default)(_context.default), 'function', 'It should be a class.');
    var myContext = new _context.default();
    assert.equal((0, _typeof2.default)(myContext), 'object', 'When instantiated it should return an object');
    assert.equal((0, _typeof2.default)(myContext.put), 'function', 'with a function for storing things,');
    assert.equal((0, _typeof2.default)(myContext.get), 'function', 'a function for getting an item we previously stored,');
    assert.equal((0, _typeof2.default)(myContext.getAll), 'function', 'and a function for getting all the items that we\'ve previously stored.');
    assert.end();
  });
  t.test('Should expose the constants', function (assert) {
    var myContext = new _context.default();
    assert.deepEqual(myContext.constants, _constants.default, 'Constants should be exposed from the context instance.');
    assert.end();
  });
  t.test('An instance should be able to store an item with a given name and return the result of the operation', function (assert) {
    var myContext = new _context.default();
    var itemToStore = {
      something: true
    };
    var anotherItemToStore = {
      somethingelse: true
    };
  ...
  t.test('An instance should be able to retrieve stored items', function (assert) {
    var myContext = new _context.default();
    var itemToStore = {
      something: true
    };
  ...
  t.test('An instance should return undefined until we have the item if we are performing a flag check.', function (assert) {
    var myContext = new _context.default();
    var value = myContext.get('test', true);
  ...
  t.test('An instance should return a promise if we try to get something we don\'t have stored unless we just want to check a flag.', function (assert) {
    var myContext = new _context.default();
    var itemPromise = myContext.get('test');
  ...
  t.test('If we store something someone is waiting for, it should resolve that promise and keep the value', function (assert) {
    var myContext = new _context.default();
    var itemToStore = {
      something: true
    };
    var itemPromise = myContext.get('test');
    setTimeout(function () {
      return myContext.put('test', itemToStore);
    }, 100);
  ...
  t.test('If we store a promise, it should store the value once it is resolved', function (assert) {
    var myContext = new _context.default();
    var itemToStore = {
      something: true
    };
    var promiseToStore = new Promise(function (res) {
      return setTimeout(function () {
        return res(itemToStore);
      }, 100);
    });
  ...
  t.test('If we store a promise, when that promise resolves it should store the value', function (assert) {
    var myContext = new _context.default();
    var itemToStore = {
      something: true
    };
    var itemPromise = new Promise(function (res) {
      return setTimeout(function () {
        return res(itemToStore);
      }, 100);
    });
    ...
    assert.equal(myContext.get('test'), itemPromise, 'If we get the value before the promise is resolved, we receive the promise.'); // This callback is attached at the end because for testing purposes, I need the one on the
  ...
  t.test('If we store a promise, but that promise fails, it should clean up that item on the context', function (assert) {
    var myContext = new _context.default();
    var err = new Error('An error has occured.');
    var itemPromise = new Promise(function (res, rej) {
      return setTimeout(function () {
        return rej(err);
      }, 100);
    });
    ...
    return itemPromise.then(function () {
      assert.fail('This should not happen on this scenario.');
      assert.end();
  ...
  t.test('Should be able to retrieve all items at once', function (assert) {
    var myContext = new _context.default();
    var item1 = {
      some: 'thing'
    };
    ...
    var _myContext$getAll = myContext.getAll(),
        test1 = _myContext$getAll.test1,
        test2 = _myContext$getAll.test2,
        test3 = _myContext$getAll.test3;
  ...
  t.test('Different instances should have different storing maps', function (assert) {
    var context1 = new _context.default();
    var context2 = new _context.default();
    var itemToStore1 = {
      something: true
    };

    var itemToStore2 = function itemToStore2() {};
