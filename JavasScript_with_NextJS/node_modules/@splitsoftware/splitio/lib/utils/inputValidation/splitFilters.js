...
function validateSplitFilter(type, values, maxLength) {
  ...
  var result = (0, _splits.validateSplits)(values, 'Factory instantiation', "".concat(type, " filter"), "".concat(type, " filter value"));
...
function validateSplitFilters(splitFilters, mode) {
  ...
  if (mode !== _constants.STANDALONE_MODE) {
    log.warn("Factory instantiation: split filters have been configured but will have no effect if mode is not '".concat(_constants.STANDALONE_MODE, "', since synchronization is being deferred to an external tool."));
    return res;
