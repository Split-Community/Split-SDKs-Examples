...
import { STANDALONE_MODE, CONSUMER_MODE } from '../../constants'; // Import the module mocking the logger.
...
tape('INPUT VALIDATION for splitFilters', function (t) {
  var defaultOutput = {
    validFilters: [],
    queryString: null,
    groupedFilters: {}
  ...
  t.test('Returns object with a queryString, if `splitFilters` contains at least a valid `byName` or `byPrefix` filter with at least a valid value', function (assert) {
    var _loop = function _loop(i) {
      if (groupedFilters[i]) {
        ...
        var output = {
          validFilters: _toConsumableArray(splitFilters[i]),
          queryString: queryStrings[i],
          groupedFilters: groupedFilters[i]
        };
        assert.deepEqual(validateSplitFilters(splitFilters[i], STANDALONE_MODE), output, "splitFilters #".concat(i));
        assert.true(loggerMock.debug.calledWith("Factory instantiation: splits filtering criteria is '".concat(queryStrings[i], "'.")));
      } else {
        // tests where validateSplitFilters throws an exception
        assert.throws(function () {
          return validateSplitFilters(splitFilters[i], STANDALONE_MODE);
        }, queryStrings[i]);
      }
    };
