...
tape('CONTEXT module', function (t) {
  t.test('Should have an API for storing and retrieving items', function (assert) {
    assert.equal(_typeof(Context), 'function', 'It should be a class.');
    var myContext = new Context();
    assert.equal(_typeof(myContext), 'object', 'When instantiated it should return an object');
    assert.equal(_typeof(myContext.put), 'function', 'with a function for storing things,');
    assert.equal(_typeof(myContext.get), 'function', 'a function for getting an item we previously stored,');
    assert.equal(_typeof(myContext.getAll), 'function', 'and a function for getting all the items that we\'ve previously stored.');
    assert.end();
  });
