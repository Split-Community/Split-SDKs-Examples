...
tape('MANAGER API / In Memory / List all splits', function (assert) {
  ...
  var manager = new Manager(cache, contextMock);
...
tape('MANAGER API / In Memory / Read only one split by name', function (assert) {
  ...
  var manager = new Manager(cache, contextMock);
  cache.addSplit(splitObject.name, JSON.stringify(splitObject));
  var split = manager.split(splitObject.name);
...
tape('MANAGER API / In Memory / Input Validation', function (assert) {
  var cache = new SplitCacheInMemory();
  var manager = new Manager(cache, contextMock);
  cache.addSplit(splitObject.name, JSON.stringify(splitObject)); // control assertions to verify that the manager is connected with that cache.
