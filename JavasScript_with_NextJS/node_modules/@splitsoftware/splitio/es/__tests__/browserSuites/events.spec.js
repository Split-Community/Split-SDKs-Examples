...
export function withoutBindingTT(fetchMock, assert) {
  ...
  assert.equal(_typeof(client.track), 'function', 'client.track should be a function.'); // Key binded as with getTreatment.
...
export function bindingTT(fetchMock, assert) {
  ...
  var splitio = SplitFactory(localSettings);
  var client = splitio.client();
  var tsStart, tsEnd;
  fetchMock.postOnce(settings.url('/events/bulk'), function (url, opts) {
    var resp = JSON.parse(opts.body); // We will test the first and last item in detail.
    ...
    assert.equal(firstEvent.value, 10, 'Value should match the value received on the .track() function.');
    assert.equal(_typeof(firstEvent.timestamp), 'number', 'The timestamp should be a number.');
    assert.equal(midEvent.key, 'facundo@split.io', 'Key should match received value.');
    assert.equal(midEvent.eventTypeId, 'genericEvent', 'EventTypeId should match received value.');
    ...
    assert.equal(lastEvent.value, null, 'Should have null as value.');
    assert.equal(_typeof(lastEvent.timestamp), 'number', 'The timestamp should be a number.');
    client.destroy();
    assert.end();
    return 200;
  });
