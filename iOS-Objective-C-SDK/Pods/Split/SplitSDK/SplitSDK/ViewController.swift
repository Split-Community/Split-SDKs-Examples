...
//            print("Treatment1: \(mySplit!.getSplitTreatment(splitName: "clients"))")
            let result = mySplit!.getSplitTreatment(splitName: "AccessCare")
            print("\nTreatment1: \(result)")
//           print("\nConfig: \(result.data)")
//           let myData = result.data
//            let config = try? JSONSerialization.jsonObject(with: result.data(using: .utf8)!, options: []) as? [String: Any]

            
//            let config = try? JSONSerialization.jsonObject(with: myData, options: []) as? [String: String]
/*            config?!.forEach { key in
