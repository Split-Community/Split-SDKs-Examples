...
function ProducerMock(splitStorage) {
  ...
  return {
    isSynchronizingSplits: _sinon.default.spy(isSynchronizingSplits),
    synchronizeSplits: _sinon.default.spy(synchronizeSplits),
...
(0, _tape.default)('SplitUpdateWorker', function (t) {
  t.test('put', function (assert) {
    // setup
    var cache = new _InMemory.default();
    var producer = ProducerMock(cache);
    var splitUpdateWorker = new _SplitUpdateWorker.default(cache, producer);
  ...
  t.test('killSplit', function (assert) {
    ...
    var producer = ProducerMock(cache);
    var splitUpdateWorker = new _SplitUpdateWorker.default(cache, producer, splitsEventEmitterMock); // assert killing split locally, emitting SDK_SPLITS_ARRIVED event, and calling `synchronizeSplits` and `killLocally`, if changeNumber is new
