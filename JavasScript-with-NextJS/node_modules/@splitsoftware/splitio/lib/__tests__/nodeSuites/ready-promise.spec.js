...
function readyPromiseAssertions(key, fetchMock, assert) {
  ...
  assert.test(function (t) {
    ...
    (0, _testUtils.mockSegmentChanges)(fetchMock, new RegExp(config.urls.sdk + '/segmentChanges/*'), ['some_key']);
    fetchMock.postOnce(config.urls.events + '/testImpressions/bulk', 200);
    fetchMock.postOnce(config.urls.events + '/testImpressions/count', 200);
    var splitio = (0, _.SplitFactory)(config);
  ...
  }, 'Time out and then ready after retry attempt'); // Time out and then ready after scheduled refresh. Time out is triggered, but the state changes into ready after refresh.
  ...
  }, 'Time out and then ready after scheduled refresh'); // Validate fallback to 'catch' callback when exception is thrown on 'then' onRejected callback.
  ...
  assert.test(function (t) {
    ...
    setTimeout(function () {
      ...
      t.true(consoleSpy.log.calledWithExactly('[ERROR] A listener was added for SDK_READY_TIMED_OUT on the SDK, which has already fired and won\'t be emitted again. The callback won\'t be executed.'), 'Logging error that a listeners for SDK_READY_TIMED_OUT event was added after triggered');
      client.destroy().then(function () {
        client.ready().then(function () {
          t.pass('### SDK IS READY - the promise remains resolved after client destruction.');
          assertGetTreatmentControlNotReadyOnDestroy(t, client, key);
  ...
  }, 'Validate that warning messages are properly sent'); // Time out event is not handled. Ready promise should not be resolved.
  ...
  assert.test(function (t) {
    ...
    fetchMock.postOnce(config.urls.events + '/testImpressions/count', 200);
    var splitio = (0, _.SplitFactory)(config);
    var client = splitio.client(); // Assert getTreatment return CONTROL and trigger warning when SDK is not ready yet
  ...
  }, 'Time out event is not handled. Ready promise should not be resolved'); // Other possible tests:
