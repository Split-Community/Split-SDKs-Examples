...
tape('MATCHER IN_SPLIT_TREATMENT / should return true ONLY when parent split returns one of the expected treatments', function (assert) {
  var matcherTrue_alwaysOn = matcherFactory({
  ...
  var matcherFalse_alwaysOn = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-on',
      treatments: ['off', 'v1']
  ...
  var matcherTrue_alwaysOff = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-off',
  ...
  var matcherFalse_alwaysOff = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-off',
      treatments: ['v1', 'on']
  ...
  assert.true(matcherTrue_alwaysOn({
    key: 'a-key'
  }, evaluateFeature), 'Parent split returns one of the expected treatments, so the matcher returns true');
  assert.false(matcherFalse_alwaysOn({
    key: 'a-key'
  }, evaluateFeature), 'Parent split returns treatment "on", but we are expecting ["off", "v1"], so the matcher returns false');
...
tape('MATCHER IN_SPLIT_TREATMENT / Edge cases', function (assert) {
  var matcherParentNotExist = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
  ...
  var matcherNoTreatmentsExpected = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-on',
      treatments: []
  ...
  var matcherParentNameEmpty = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: '',
      treatments: []
  ...
  var matcherParentNameWrongType = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: {
        some: 44
  ...
  var matcherExpectedTreatmentWrongType_matching = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-on',
  ...
  var matcherExpectedTreatmentWrongType_notMatching = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-off',
      treatments: [null, [1, 2], 3, {}, true, 'on']
  ...
  var matcherExpectationsListWrongType = matcherFactory({
    type: matcherTypes.IN_SPLIT_TREATMENT,
    value: {
      split: 'always-on',
      treatments: 658
  ...
  assert.false(matcherParentNotExist({
    key: 'a-key'
  }, evaluateFeature), 'If the parent split does not exist, matcher should return false');
  assert.false(matcherNoTreatmentsExpected({
    key: 'a-key'
  }, evaluateFeature), 'If treatments expectation list is empty, matcher should return false (no treatment will match)');
  assert.false(matcherParentNameEmpty({
    key: 'a-key'
  }, evaluateFeature), 'If the parent split name is empty, matcher should return false');
  assert.false(matcherParentNameWrongType({
    key: 'a-key'
  }, evaluateFeature), 'If the parent split name is not a string, matcher should return false');
  assert.true(matcherExpectedTreatmentWrongType_matching({
    key: 'a-key'
  }, evaluateFeature), 'If treatments expectation list has elements of the wrong type, those elements are overlooked.');
  assert.false(matcherExpectedTreatmentWrongType_notMatching({
    key: 'a-key'
  }, evaluateFeature), 'If treatments expectation list has elements of the wrong type, those elements are overlooked.');
  assert.false(matcherExpectationsListWrongType({
    key: 'a-key'
  }, evaluateFeature), 'If treatments expectation list has wrong type, matcher should return false');
  assert.end();
});