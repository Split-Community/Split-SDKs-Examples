...
import { _Set, setToArray } from '../../../utils/lang/Sets'; // The list of methods we're wrapping on a promise (for timeout) on the adapter.

var METHODS_TO_PROMISE_WRAP = ['set', 'exec', 'del', 'get', 'keys', 'sadd', 'srem', 'sismember', 'smembers', 'incr', 'rpush', 'pipeline', 'expire', 'mget'];
var ioredisMock = reduce([].concat(METHODS_TO_PROMISE_WRAP, ['disconnect']), function (acc, methodName) {
...
var timeout = sinon.spy(function timeout(ms, originalPromise) {
  var resolvers = {};
...
var RedisAdapter = proxyquire('../../RedisAdapter', {
  'ioredis': ioredis,
  '../utils/logger': {
    default: LogFactory
  },
...
tape('STORAGE Redis Adapter / Class', function (assert) {
  ...
  assert.true(_typeof(instance._options) === 'object', 'The instance will have an options object.');
...
tape('STORAGE Redis Adapter / ioredis constructor params and static method _defineLibrarySettings', function (assert) {
  ...
  new RedisAdapter(_objectSpread(_objectSpread({}, redisParams), {}, {
    connectionTimeout: 123,
    operationTimeout: 124
  }));
...
tape('STORAGE Redis Adapter / static method - _defineOptions', function (assert) {
  var defaultOptions = {
    connectionTimeout: 10000,
...
tape('STORAGE Redis Adapter / instance methods - _listenToEvents', function (assert) {
  // Reset all stubs
  sinon.resetHistory();
  ...
  assert.equal(_typeof(firstCall.args[1]), 'function', 'second argument for the first call should be a callback function.');
  ...
  assert.equal(_typeof(secondCall.args[1]), 'function', 'second argument for the first call should be a callback function.');
  ...
  assert.true(Array.isArray(instance._notReadyCommandsQueue), 'Control assertion'); // Without any offline commands queued, execute the callback for "ready"
...
tape('STORAGE Redis Adapter / instance methods - _setTimeoutWrappers and queueing commands 1/2 - Error path', function (assert) {
  sinon.resetHistory();
  var instance = new RedisAdapter({
    url: 'redis://localhost:6379/0'
  });
  forEach(METHODS_TO_PROMISE_WRAP, function (methodName) {
    ...
    assert.equal(_typeof(queuedCommand.resolve), 'function', 'The queued item should have the correct form.');
    assert.equal(_typeof(queuedCommand.reject), 'function', 'The queued item should have the correct form.');
    assert.equal(_typeof(queuedCommand.command), 'function', 'The queued item should have the correct form.');
...
tape('STORAGE Redis Adapter / instance methods - _setTimeoutWrappers and queueing commands 2/2 - Success path', function (assert) {
  sinon.resetHistory();
  var instance = new RedisAdapter({
    url: 'redis://localhost:6379/0'
  });
...
tape('STORAGE Redis Adapter / instance methods - _setDisconnectWrapper', function (assert) {
  sinon.resetHistory();
  var instance = new RedisAdapter({
    url: 'redis://localhost:6379/0'
  });
  ...
  setTimeout(function () {
    ...
    instance.disconnect();
    var rejectedPromise = Promise.reject('test-error');
    ...
    setTimeout(function () {
      ...
      Promise.all(setToArray(instance._runningCommands)).catch(function (e) {
