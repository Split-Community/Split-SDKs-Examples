...
export default function readyPromiseAssertions(fetchMock, assert) {
  ...
  assert.test(function (t) {
    ...
    fetchMock.get(config.urls.sdk + '/mySegments/facundo%40split.io', mySegmentsFacundo, {
      delay: fromSecondsToMillis(config.startup.requestTimeoutBeforeReady) - 20
    });
  ...
  assert.test(function (t) {
    ...
    fetchMock.get(config.urls.sdk + '/mySegments/nicolas%40split.io', mySegmentsFacundo); // shared client endpoint configured to emit SDK_READY_TIMED_OUT
