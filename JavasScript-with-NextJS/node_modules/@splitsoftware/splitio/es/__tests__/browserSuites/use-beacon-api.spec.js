...
var assertImpressionsCountSent = function assertImpressionsCountSent(assert, impressionCount) {
  ...
  assert.equal(_typeof(impressionCount.pf[0].m), 'number', 'It should be number.');
};
...
function beaconApiNotSendTest(fetchMock, assert) {
  sendBeaconSpy = sinon.spy(window.navigator, 'sendBeacon'); // Mocking this specific route to make sure we only get the items we want to test from the handlers.
